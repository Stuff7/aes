/// Length of the key expansion array used in AES.
///
/// This constant represents the total number of bytes in the key expansion,
/// which is derived from the original encryption key to create round keys.
pub const KEY_EXPANSION_LEN: usize = 240;

/// Number of 32-bit words in an AES encryption key.
///
/// The AES encryption key is organized into a number of 32-bit words,
/// for 256 bits this value is 256 / 32 = 8
pub const KEY_WORD_COUNT: usize = 8;

/// Number of rounds in the AES cipher.
///
/// The AES algorithm operates through a fixed number of rounds, and this
/// constant represents the total number of rounds used in the encryption process.
pub const ROUND_COUNT: usize = 14;

/// Number of columns in the AES state.
///
/// The AES state is a two-dimensional array organized as a grid of bytes.
pub const STATE_COLUMNS: usize = 4;

/// Length of a block in bytes in the AES algorithm.
///
/// In AES, the data is processed in fixed-size blocks, and this constant represents the size of each block in bytes.
pub const BLOCK_LEN: usize = 16;

/// Substitution box used in the AES encryption process.
///
/// This is a lookup table used to substitute each byte in the state during encryption.
/// The values in this array are to provide non-linearity and diffusion in the encryption process.
pub const SUBSTITUTION_BOX: [u8; 256] = [
...
/// Inverse substitution box used in the AES decryption process.
///
/// Similar to the substitution box, this array is used during decryption to substitute bytes in the reverse direction.
pub const INV_SUBSTITUTION_BOX: [u8; 256] = [
...
/// Round constants used in the AES key expansion process.
///
/// These constants are used in the key expansion process to introduce
/// variability and non-linearity into the derived round keys.
pub const ROUND_CONSTS: [u8; 11] = [0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36];
